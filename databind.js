/*
 * Two-way data binding for faster performance and large grids. While not architected as elegantly as Vue, the bindings here
 * lightweight and the rendering is much faster with large grids that potentially show thousands of rows. Traditionally, such lists are either
 * thrown behind an infinite scrollable list or paginated list, but we'll resist them as far as we can. Showing all values results in simpler user
 * interfaces where the searching, sorting, and generally finding the data you want are much simplified.
 * This is based on https://github.com/ken107/databind-js
 */
(function() {
	var regex = [
		/{@[\s\S]*?@}/g,
			...
			if (newCount > count) {
				...
				for (var i=count; i<newCount; i++) {
					if (cache.firstChild) {
						...
						bindNow(newElem, newData, context, bindingStore, debugInfo);
					}
				}
	...
	function bindNow(node, data, context, bindingStore, debugInfo) {
		if (node.nodeType == 1 && node.tagName != "TEMPLATE") {
			...
			if (dirs.repeater) {
				...
				while (child) {
					...
					if (child.nodeType == 1 || child.nodeType == 3 && child.nodeValue.indexOf('{@') != -1) bindNow(child, data, context, bindingStore, debugInfo);
					child = nextSibling;
				}
			}
		}
	...
	var api = {
		...
		binder: function(elem, context, bindingStore, debugInfo) {
			bindNow(elem, context, context, bindingStore||new BindingStore(), debugInfo||[]);
		},
	...
	function onReady() {
			if (api.autoBind) {
				...
				api.binder(document.body, window, null, ["document"]);
